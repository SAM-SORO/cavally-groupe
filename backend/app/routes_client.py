...
from .schemas_client import ClientSortie, ConnexionEntree, DemandeSortie, InscriptionEntree
...
def connexion(entree: ConnexionEntree, reponse: Response, db: Session = Depends(get_db)) -> Client:
    ...
    if client is None or not verifier_mot_de_passe(entree.mot_de_passe, client.mot_de_passe_hash):
...
async def deposer(
    ...
    Aucune conversion, aucune extraction, aucun enregistrement : le fichier est
    transmis tel quel puis ecarte. C'est l'entreprise qui le traitera ensuite
